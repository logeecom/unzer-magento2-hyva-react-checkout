...
export default function UnzerBancontact({ method, selected, actions }) {
  const methodCode = method?.code || 'unzer_bancontact';
  ...
  useEffect(() => {
    ...
    const handler = async (values) => {
      ...
      try {
        ...
        console.error('[UnzerBancontact] Error:', err);
        setErrorMessage(
            err?.message || 'Unable to process Bancontact payment.'
        );
        return false;
      } finally {
        submittingRef.current = false;
