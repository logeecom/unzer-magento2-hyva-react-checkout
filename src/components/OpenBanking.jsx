...
export default function UnzerOpenBanking(props) {
  const methodCode = props.method?.code || 'unzer_open_banking';
  ...
  const onPlaceOrder = async ({ values, checkoutEl }) => {
    if (!checkoutEl) {
      setErrorMessage('Open Banking form not ready.');
      return false;
    }
    ...
    try {
      ...
      console.error('[UnzerOpenBanking]', err);
      setErrorMessage(err?.message || 'Unable to process Open Banking.');
      return false;
    } finally {
      setPageLoader(false);
    }
