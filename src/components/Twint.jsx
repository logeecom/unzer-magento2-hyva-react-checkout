...
/**
 * Unzer TWINT – Redirect Component
 */
export default function UnzerTwint({ method, selected, actions }) {
  const methodCode = method?.code || 'unzer_twint';
  ...
  const mountUnzerElements = useCallback(() => {
    ...
    try {
      ...
      console.warn('[UnzerTwint] snapshot refresh failed:', e);
    }
  ...
  useEffect(() => {
    ...
    const handler = async (values) => {
      ...
      try {
        ...
        console.error('[UnzerTwint Error]', err);
